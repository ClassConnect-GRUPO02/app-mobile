...
import { Text, Card, Button, ActivityIndicator, Divider, TextInput, Chip, IconButton } from "react-native-paper"
...
export default function TaskSubmissionsScreen() {
    ...
    useEffect(() => {
        const fetchTaskAndSubmissions = async () => {
            try {
                ...
                if (!instructorStatus) {
                    throw new Error("No tienes permisos para ver las entregas de esta tarea")
                }
                ...
                const submissionsData = await taskClient.getTaskSubmissions(courseId, currentUserId, taskId)
                ...
                const submissionsWithNames = await Promise.all(
                    submissionsData.map(async (submission: Submission) => {
                        try {
                            const studentInfo = await userApi.getUserById(submission.student_id)
                            return {
                                ...submission,
                                student_name: studentInfo?.user?.name || "Estudiante desconocido",
                            }
                        } catch (error) {
    ...
    const handleSaveFeedback = async () => {
        ...
        try {
            ...
            Alert.alert("Error", "No se pudo guardar la retroalimentación. Inténtalo de nuevo.")
        } finally {
            setSubmitting(false)
        }
    }
    ...
    if (!isInstructor) {
        return (
            <View style={styles.errorContainer}>
                ...
                <Text style={styles.errorText}>No tienes permisos para ver las entregas de esta tarea.</Text>
                <Button mode="contained" onPress={handleBack} style={styles.backButton}>
                    Volver
                </Button>
            </View>
        ...
        <View style={styles.container}>
            ...
            {task && (
                <View style={styles.taskInfo}>
                    ...
                    <Text style={styles.dueDate}>Fecha de entrega: {new Date(task.due_date).toLocaleDateString()}</Text>
                </View>
            )}
            ...
            <View style={styles.content}>
                {submissions.length === 0 ? (
                    <View style={styles.emptyContainer}>
                        ...
                        <Text style={styles.emptyText}>Aún no se han recibido entregas de los estudiantes.</Text>
                    </View>
                ) : (
                    <ScrollView style={styles.submissionsList}>
                        ...
                        {submissions.map((submission) => (
                            <Card
                                key={submission.id}
                                style={[styles.submissionCard, selectedSubmission?.id === submission.id && styles.selectedCard]}
                                ...
                                <Card.Content>
                                    <View style={styles.submissionHeader}>
                                        ...
                                        <Chip
                                            style={[
                                                styles.statusChip,
                                                { backgroundColor: submission.status === "late" ? "#ffebee" : "#e8f5e9" },
                                            ]}
                                            textStyle={{
                                                color: submission.status === "late" ? "#c62828" : "#2e7d32",
                                            }}
                                        >
                                            {submission.status === "late" ? "Entrega tardía" : "Entregado a tiempo"}
                                        </Chip>
                                    </View>
                                    ...
                                    <View style={styles.gradeContainer}>
                                        ...
                                        >
                                            {submission.grade !== null ? submission.grade.toFixed(1) : "Sin calificar"}
                                        </Text>
                                    </View>
