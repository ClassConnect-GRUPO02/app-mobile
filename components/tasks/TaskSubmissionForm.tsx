...
export const TaskSubmissionForm: React.FC<TaskSubmissionFormProps> = ({
  ...
  const validateSubmission = (): boolean => {
    ...
    if (task.type === "examen" && examExpired) {
      Alert.alert(
        "Error",
        "El tiempo del examen ha expirado. No puedes enviar respuestas."
      );
      return false;
    }
    ...
    try {
      ...
      if (selectedFile) {
        try {
          ...
          fileUrl = await supabaseClient.uploadFile(
            selectedFile.uri,
            courseId,
            selectedFile.name
          );
      ...
      const result = await taskClient.submitTask(
        courseId,
        task.id,
        userId,
        submissionAnswers,
      ...
      if (result) {
        if (timerRef.current) clearInterval(timerRef.current);
        // 🔥 Llamada automática a la autocorrección con IA
        if (task.answer_format === "preguntas_respuestas") {
          try {
            await taskClient.gradeTaskWithAI(task.id, userId);
            console.log("✅ Corrección automática completada");
          } catch (autoCorrectError) {
            console.warn("⚠️ No se pudo autocorregir:", autoCorrectError);
          }
        }
    ...
    return (
      <View style={styles.completionContainer}>
        ...
        <Text style={styles.completionText}>
          Ya has enviado tu respuesta para este examen. No puedes realizar
          cambios adicionales.
        </Text>
        <Button
          mode="outlined"
          onPress={onSubmissionComplete}
    ...
    return (
      <ScrollView style={styles.container}>
        <View style={styles.taskInfo}>
          ...
          <Chip
            style={[styles.typeChip, { backgroundColor: "#fff3e0" }]}
          ...
          {task.time_limit_minutes && (
            <Text style={styles.dueDate}>
              Tiempo límite: {task.time_limit_minutes} minutos
            </Text>
          )}
          ...
          <View style={styles.examStartSection}>
            ...
            <Text style={styles.examWarningText}>
              Este es un examen con tiempo límite de {task.time_limit_minutes}{" "}
              minutos. Una vez que inicies el examen, el tiempo comenzará a
              correr y no se puede pausar.
            </Text>
      ...
      {task.type === "examen" && task.has_timer && timeRemaining !== null && (
        <View style={styles.timerContainer}>
          <Text
            style={[
              styles.timerText,
              timeRemaining < 300 ? styles.timerWarning : null,
            ]}
          >
        ...
        >
          <View style={styles.modalContent}>
            ...
            <Text style={styles.warningText}>
              Te quedan menos de 5 minutos para completar este examen. Asegúrate
              de enviar tu respuesta antes de que se acabe el tiempo.
            </Text>
